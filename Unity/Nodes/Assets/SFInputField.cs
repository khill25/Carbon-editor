...
namespace UnityEngine.UI {
		...
		public ScrollRect currentScrollRect;
			...
			public bool m_AllowInput = false;
			...
			public List<ISFEditingDelegate> editingDelegates = new List<ISFEditingDelegate>();
		public void subscribeToEditingUpdates(ISFEditingDelegate obj) {
			if (!editingDelegates.Contains (obj)) {
				editingDelegates.Add (obj);
			}
		}

		public void unsubscribeToEditingUpdates(ISFEditingDelegate obj) {
			if (editingDelegates.Contains (obj)) {
				editingDelegates.Remove(obj);
			}
		}

			protected SFInputField(){}
			...
			foreach (ISFEditingDelegate note in editingDelegates) {
				//ExecuteEvents.Execute<ISFEditingDelegate>(note.getGameObject(), null, (x,y)=>x.setIsEditingDelegateValue(false));
				note.getGameObject().BroadcastMessage("setIsEditingDelegateValue", true);
			}
		}
			...
			public void DeactivateInputField()
			{
				...
				foreach (ISFEditingDelegate note in editingDelegates) {
					//ExecuteEvents.Execute<ISFEditingDelegate>(note.getGameObject(), false, (x,y)=>x.setIsEditingDelegateValue());
					note.getGameObject().BroadcastMessage("setIsEditingDelegateValue", false);
				}
		}
